...
void setup() {
  ...
  if (!Sensor_Configuration()) {
    Serial.println(
        "[BOOT] WARNING: One or more sensors failed — check wiring!");
  }

  // MPU6050 Gyro (Must be initialized AFTER Sensor_Configuration because it
  // relies on Wire.begin())
  if (!Gyro_Init()) {
  ...
  Serial.println(
      "[BOOT] Init complete. Robot ready. Waiting 5 seconds before start...");
...
void loop() {
  ...
  // ---- Print telemetry ----
  Serial.printf("[TEL] 90R=%4d 45R=%4d 0R=%4d 0L=%4d 45L=%4d 90L=%4d  "
  ...
  // booleans
  uint16_t frontAvg =
      (sensorMM[SENSOR_0L] == 9999 || sensorMM[SENSOR_0R] == 9999)
          ? 9999
